...
encoder.load_state_dict(torch.load("models/wuyan/seq2seq_attn_bigru_encoder.pth",map_location=torch.device('cpu')))
decoder.load_state_dict(torch.load("models/wuyan/seq2seq_attn_bigru_decoder.pth",map_location=torch.device('cpu')))
encoder.eval()
decoder.eval()

generate(encoder, decoder, 10, input_verse)
'''
...
def generate(encoder, decoder, num_topic, input_verse):
    return_list=[]
    if input_verse == None:
        print("请输入一句五言诗句!")
    ...
    for j in range(num_topic):
        ...
        print("topic %d" % (j))
        print("-" * 40)
        for idx, each in enumerate(beam_res_list[j]):
            print("No.", idx, sep="")
            poem_lst = each.poem
            print(poem_lst)
            ...
            for i in range(1,len(poem_lst)):
                print(poem_lst[i], end="")
                if j in {1,5,9} and idx==0:
                    return_list.append(poem_lst[i])
                if (poem_lst[i] == "，")or (poem_lst[i]=="。"):
                    print("")
            print("")
            print("posb:", each.posb)
...
def jiekou(first_sentence):
    first_sentence = str2tensor(first_sentence)
    encoder = Encoder(len(wd2Idx), hidden_size,vec_dim,num_layer)
    decoder = Decoder(len(wd2Idx), hidden_size,vec_dim,num_layer,num_topic,dropout_p=0.1)
    encoder = encoder.to(device)
    decoder = decoder.to(device)
    ...
    return generate(encoder, decoder, 10, first_sentence)
